...
out_dir = '/projectnb/bioinfor/SEGRE/goldford/CoenzymeSpecificity/pytfa/tests/FVA/11Oct2021_v2/SC'
...
cobra_model = load_json_model(model_path + '/iJO1366_NAD_semi-unconstrained.11Oct2021.json')
...
print("finished variability analysis for optimal solution")

# perform variability analysis on reactions
mytfa.reactions.get_by_id(biomass_rxn).lower_bound = 0.1
varib_results = variability_analysis(mytfa,kind='reactions')
varib_results.to_csv(out_dir + '/FVA.sc.results.gtzp1.csv')
print("finished variability analysis with growth rate = 0.1")

# perform variability analysis on reactions
mytfa.reactions.get_by_id(biomass_rxn).lower_bound = 0.5
varib_results = variability_analysis(mytfa,kind='reactions')
print("finished variability analysis with growth rate = 0.5")
